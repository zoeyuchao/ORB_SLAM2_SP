...
#  3. 一定要记得！！！更换数据集后，把png txt文件夹下的文件全部拷出来单独存，尤其是txt文件，否则分不清是哪个数据集测评的结果了。由于记录下的txt文件里的数据之间都有空格作为分隔符，所以可以直接粘贴到libreOffice软件 的excel表格里面，选择分隔选项�?空格 就可以完美整理到表格中啦�?不过png 和txt文件夹不要删除哦，否则无法自动创建的�?#  4. 在终端运�?./orbEvaluate.sh 即可
